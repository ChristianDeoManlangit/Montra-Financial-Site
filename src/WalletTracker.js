...
const WalletTracker = ({ onSettingsClick, userProfileImage }) => {
  ...
  // Calculate unfrozen wallets and totals - FIXED to include frozen accounts
  const getUnfrozenWallets = () => wallets.filter(w => !w.frozen);
  const totalNetWorth = wallets.reduce((sum, wallet) => sum + wallet.balance, 0); // Include ALL wallets
    ...
    <div className="min-h-screen bg-[#F1F3FF] flex">
      ...
      <div className="flex-1 lg:ml-64 flex flex-col">
        ...
        <header className="sticky top-0 z-30 bg-white border-b border-gray-200 p-3 sm:p-6 flex justify-between items-center animate-fadeIn">
          ...
          <div className="flex items-center gap-4">
            <button onClick={onSettingsClick} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200"><Settings size={20} className="text-[#1E1E1E]" /></button>
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 hover:bg-gray-100 rounded-full">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center cursor-pointer overflow-hidden border-2 border-white shadow-lg" style={{backgroundImage: userProfileImage ? `url(${userProfileImage})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center'}}>
                {!userProfileImage && <User size={24} className="text-white" />}
              </div>
            </button>
            <button className="hidden lg:flex p-2 hover:bg-gray-100 rounded-full">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center overflow-hidden border-2 border-white shadow-lg" style={{backgroundImage: userProfileImage ? `url(${userProfileImage})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center'}}>
                {!userProfileImage && <User size={24} className="text-white" />}
              </div>
            </button>
          </div>
          ...
          <div className="max-w-7xl mx-auto">
            {activeTab === "dashboard" && (
              <div className="space-y-3 sm:space-y-6 animate-fadeIn">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold text-[#1E1E1E]">Analytics</h3>
